...
double PnpSolver:: oupost_reprojection_error(Armor armor, const double & pitch)
{
  ...
  const auto & object_points =
    (PnpSolver::Islarge(armor)) ? BIG_ARMOR_POINTS : SMALL_ARMOR_POINTS;

  cv::Vec3d rvec, tvec;
  cv::solvePnP(
    object_points, armor.corners, camera_matrix_, distort_coeffs_, rvec, tvec, false,
    cv::SOLVEPNP_IPPE);
